...
bool parseReview(const string &line, Review &r) {
    ...
    // Handle if review text has commas inside (combine back)
...
// Filter transactions by category and payment method
...
// Analyze 1-star reviews (frequent words)
void analyze1StarReviews() {
    ...
    for (int i = 0; i < reviewCount; ++i) {
        if (reviews[i].rating == 1) {
            ...
            while (ss >> word) {
                ...
                if (word.length() > 2) { // Ignore very small words like "a", "is"
                    wordFrequency[word]++;
                }
            }
        }
    ...
    for (auto &p : sortedWords) {
        ...
        if (++displayed >= 10) break; // Display top 10
    }
}
...
int main() {
    ...
    while (true) {
        ...
        cout << "3. Analyze 1-star reviews (frequent words)\n";
